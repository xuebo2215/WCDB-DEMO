...
{
    return [self.wcdb deleteObjectsFromTable:NSStringFromClass(self) where:[self formatCondition:condition]];
}

+ (BOOL)wcdb_deleteall
{
    return [self.wcdb deleteAllObjectsFromTable:NSStringFromClass(self)];
}
